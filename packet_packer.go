...
func (p *packetPacker) appendShortHeaderPacket(
	...
	// Handle packet based on the encryption.
	// If the packet is sent unencrypted, no cryptographic function is applied on the packet body or header.
	// We add the 0 instead of the AEAD tag to have the same format for the packet
	if DisableEncryption && p.perspective == protocol.PerspectiveClient && ClientFirstPacketSentAndReceived {
		raw = raw[:len(raw)+sealer.Overhead()]
	} else if DisableEncryption && p.perspective == protocol.PerspectiveServer && ServerFirstPacketSentAndReceived {
		raw = raw[:len(raw)+sealer.Overhead()]
	} else {
